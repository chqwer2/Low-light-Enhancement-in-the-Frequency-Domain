...
from network.attention import Atten
...
class Rec_Conv_block(tf.keras.layers.Layer):
    def __init__(self, num_filters=200, kernel_size=3, use_bias=False, **kwargs):
        ...
        self.num_filters = num_filters
        self.kernel_size = kernel_size
        he_initializer = tf.keras.initializers.VarianceScaling(scale=2.0, mode='fan_in',
                                                               distribution='truncated_normal')  # stddev = sqrt(scale / n), 'uniform'
        kernel_initializer = 'glorot_uniform'  # he_initializer  #
        self.conv_0 = Conv2D(filters=self.num_filters, kernel_size=1, use_bias=use_bias,
                             # kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                             kernel_initializer=kernel_initializer, padding='same')
        self.conv_1 = Conv2D(filters=self.num_filters, kernel_size=self.kernel_size,
                             kernel_initializer=kernel_initializer,
                             # kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                             use_bias=use_bias, padding='same')
        self.conv_2 = Conv2D(filters=self.num_filters, kernel_size=self.kernel_size,
                             kernel_initializer=kernel_initializer,
                             # kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                             use_bias=use_bias, padding='same')
        self.conv_3 = Conv2D(filters=self.num_filters, kernel_size=self.kernel_size,
                             kernel_initializer=kernel_initializer,
                             # kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                             use_bias=use_bias, padding='same')
        self.conv_4 = Conv2D(filters=self.num_filters, kernel_size=self.kernel_size,
                             kernel_initializer=kernel_initializer,
                             # kernel_regularizer=tf.keras.regularizers.l2(1e-4),
                             use_bias=use_bias, padding='same')
        momentum = 0.95
...
class Asy_Rec_Conv_block(tf.keras.layers.Layer):
    def __init__(self, num_filters=200, kernel_size=3, use_bias=False, **kwargs):
        ...
        self.bn_out_X = BatchNormalization(momentum=momentum)
