...
if __name__ == '__main__':
    ...
    im1 = tf.convert_to_tensor(np.array(Image.open(r'C:\Users\calvchen\Downloads\stage1\Our_low\low00001.png')
                                        , np.float32) / 255, dtype=tf.float32)
    im2 = tf.convert_to_tensor(np.array(Image.open(r'C:\Users\calvchen\Downloads\stage1\Our_low\low00010.png')
                                        , np.float32) / 255, dtype=tf.float32)
    # 0.0  -< 0.00944 -< 0.62513345
    distance = lpips(im1, im2)
    # output tf1 tensor
    sess = tf.compat.v1.Session()
    print("LPIPS:",sess.run(distance))
